...
void motors_cb( const ntbd_msgs::Motors_Array& angles_msg){
  int f;
  for(f=0; f < STEPPER_COUNT; f++) {
    if(angles_msg.data[f] != stepperTarget[f]) {
      stepperTarget[f] = -angles_msg.data[f];
      stepperNewTarget[f] = true;    
    }    
  }
  
  servo1.write(angles_msg.data[3]);
  
}

// Subscriber node declaration, specifies the topic to which subscribe and the callback funtion
ros::Subscriber<ntbd_msgs::Motors_Array> sub("motors", motors_cb);
